...
global {
	...
	matrix level_series <- file("../includes/LevelSeriesSc4.csv");
	...
	bool aux_level_zero update:(current_level = 0);
	...
	//string scenario <- "S1-integ" ;//all CP with bdi 
	//string scenario <- "S2-integ";//all NC with bdi
	//string scenario <- "S3-integ"; //baseline + average level + neighbourhood apenas CI
	string scenario <- "S4-integ"; //swap CP and NC + average level + neighbourhood apenas CI 
...
initial state****************/
	...
	reflex save_daily_data when: cycle > 0{	
		save [cycle,int(self),my_day,all_pumps_daily_withdrawal] to: "../results/Twoway-v2/daily_withdrawal"+scenario+".csv"  type:csv rewrite:false header:false;				 
...
species Farmer control: simple_bdi {
	...
	predicate water_scarcity <- new_predicate("No water level observed");
	...
	//Water scarcity belief
	reflex water_scarcity{
		if aux_level_zero {
			do add_belief(trigger_restriction_rule); //farmers stop withdrawal when water level is zero
		}
	}
	...
	reflex neigh_effect{	
		if scenario = 'S3-integ' or scenario = 'S4-integ'{
			if profile = 'CI' {
